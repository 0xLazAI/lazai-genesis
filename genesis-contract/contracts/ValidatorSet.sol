...
import "./interface/0.6.x/IValidatorSet.sol";
...
contract ValidatorSet is IValidatorSet, System, IParamSubscriber {
     ...
     * 
     * Malaketh-Layered: If empty arrays are passed, automatically fetch validator data from StakeHub
     */
    function updateValidatorSetV2(
        ...
        // Malaketh-Layered: Auto-fetch from StakeHub if empty arrays are provided
        if (_consensusAddrs.length == 0) {
            uint256 totalLength;
            uint256[] memory votingPowersUint256;
            bytes[] memory tendermintPubKeys;
            
            // Fetch validator election info from StakeHub
            (_consensusAddrs, votingPowersUint256, _voteAddrs, tendermintPubKeys, totalLength) = 
                IStakeHub(STAKE_HUB_ADDR).getValidatorElectionInfo(0, 0);
            
            // Convert uint256[] to uint64[] for voting powers
            _votingPowers = new uint64[](_consensusAddrs.length);
            for (uint256 i; i < _consensusAddrs.length; ++i) {
                _votingPowers[i] = uint64(votingPowersUint256[i] / 10_000_000_000);
            }
            // Note: tendermintPubKeys are fetched but not used in ValidatorSet
            // They are stored in StakeHub and accessed via getValidatorElectionInfo()
        }
