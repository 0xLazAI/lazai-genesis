...
{
    /*----------------- constants -----------------*/
    string private constant NAME = "Governance Token";
    string private constant SYMBOL = "govMETIS";
    ...
    /*----------------- external functions -----------------*/
    /**
     * @dev Sync the account's govMETIS amount to the actual METIS value of the StakingCredit he holds
    ...
    /**
     * @dev Batch sync the account's govMETIS amount to the actual METIS value of the StakingCredit he holds
    ...
    /**
     * @dev delegate govMETIS votes to delegatee
    ...
    function _sync(address stakeCredit, address account) internal {
        uint256 latestMETISAmount = IStakeCredit(stakeCredit).getPooledMETIS(account);
        uint256 _mintedAmount = mintedMap[stakeCredit][account];

        if (_mintedAmount < latestMETISAmount) {
            uint256 _needMint = latestMETISAmount - _mintedAmount;
            mintedMap[stakeCredit][account] = latestMETISAmount;
            _mint(account, _needMint);
        } else if (_mintedAmount > latestMETISAmount) {
            uint256 _needBurn = _mintedAmount - latestMETISAmount;
            mintedMap[stakeCredit][account] = latestMETISAmount;
